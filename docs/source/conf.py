...
man_pages = [(master_doc, "jaxlie", "jaxlie documentation", [author], 1)]
...
def _gen_name_aliases():
    ...
    def recurse(module, prefixes):
        ...
        for member_name in dir(module):
            if member_name == "jaxlie":
                continue

            member = getattr(module, member_name)
            if callable(member):
                full_name = ".".join(["jaxlie"] + prefixes + [member_name])

                shortened_name = "jaxlie"
                current = jaxlie
                success = True
